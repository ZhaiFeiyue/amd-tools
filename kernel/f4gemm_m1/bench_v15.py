#!/usr/bin/env python3
"""Benchmark v15 with FULLY preshuffled inputs (same format as CK/ASM)."""
...
# Compile
src = f"{KDIR}/f4gemm_m1_v15.cpp"
SO = f"{KDIR}/f4gemm_m1_v15.so"
...
# Shuffled path (for v15 and CK/ASM)
...
print(f"  v15[:8]:    {D_hip[0,:8].float().cpu().tolist()}")
...
ms_v15 = triton.testing.do_bench(run_v15, warmup=15, rep=80)
tflops_v15 = 2.0 * M * N * K / ms_v15 * 1e-9
print(f"\nv15 preshuffled: {ms_v15:.4f} ms, {tflops_v15:.2f} TFLOPS")
...
print(f"\n{'='*50}")
print(f"v15 vs CK:  {tflops_v15/tflops_ck:.3f}x")
print(f"v15 vs ASM: {tflops_v15/tflops_asm:.3f}x")
