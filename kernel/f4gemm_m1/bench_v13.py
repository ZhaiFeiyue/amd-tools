#!/usr/bin/env python3
"""Benchmark v13 with FULLY preshuffled inputs (same format as CK/ASM)."""
...
# Compile
src = f"{KDIR}/f4gemm_m1_v13.cpp"
SO = f"{KDIR}/f4gemm_m1_v13.so"
...
# Shuffled path (for v13 and CK/ASM)
...
print(f"  v13[:8]:    {D_hip[0,:8].float().cpu().tolist()}")
...
ms_v13 = triton.testing.do_bench(run_v13, warmup=15, rep=80)
tflops_v13 = 2.0 * M * N * K / ms_v13 * 1e-9
print(f"\nv13 preshuffled: {ms_v13:.4f} ms, {tflops_v13:.2f} TFLOPS")
...
print(f"\n{'='*50}")
print(f"v13 vs CK:  {tflops_v13/tflops_ck:.3f}x")
print(f"v13 vs ASM: {tflops_v13/tflops_asm:.3f}x")
